...
- `docs/medium.html`: the Medium-import page generated by Pandoc. It emits
  actual equation `<img>` tags via Pandoc `--webtex`, renders those equation
  SVGs locally with MathJax into `docs/assets/`, and rewrites the page to
  same-site image paths so Medium sees images instead of raw TeX, MathML, or
  script-rendered MathJax.
