...
- `docs/medium.html`: the Medium-import page generated by Pandoc. It emits
  readable text for inline math and PNG `<img>` tags for display equations.
  The PNGs are rendered locally with MathJax/Sharp into `docs/assets/`, and the
  page uses absolute GitHub Pages image URLs so Medium sees normal supported
  images instead of raw TeX, SVG, MathML, or script-rendered MathJax.
